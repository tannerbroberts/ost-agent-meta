...
## 2026-09-15 — second instance of the contradicting-declarations failure, so it is a class; and this time the expensive failure direction actually fired

Short. This is the replication the 2026-09-09 section above explicitly asked for ("a second instance would be needed to call it a class rather than an event"), plus the one thing that section could only predict.

**The replication.** This firing's prompt carried the same two generated blocks. The `What this surface withholds` list named seven tools — `ost_check`, `ost_debt`, `ost_deposit`, `ost_flag_humans_required`, `ost_gate`, `ost_rank_source`, `ost_status` — and again did not name `ost_read_repo`. The hard-rules paragraph again asserted the opposite in prose: "This unattended sweep holds no outward-sensing grant on purpose — `ost_search_web` / `ost_read_web` / `ost_read_repo` stay on the attended path." Same two declarations, same disagreement, same direction. Two instances eleven days apart, both on this vault's loop, so by this node's own stated criterion it is a class.

**What is new: the predicted harm was not hypothetical this time.** The 2026-09-09 section argued the dangerous direction is a pass that believes the prose, declines repo sight, and therefore declines to instrument rather than write against invented paths — naming `ost.config.yaml`'s `product.repos` key, set 2026-08-09 after "six consecutive discovery passes" did exactly that. This pass believed the prose. It went looking for the product's spec suite through the only route it thought it had, composing `Glob` on `/Users/tanner/dev/OST-Agent/test` and then `Grep` on the same directory; both were denied for want of a read grant. It was on the point of reporting `solutionsMissingInstruments` as unworkable-by-design when the 2026-09-09 section above — read for an unrelated reason — said the grant was probably there. One `ost_read_repo` call listed `test/` (31 entries) without difficulty.

**The measured cost, which is higher than the first instance's.** The first instance cost one speculative call. This one cost two denied calls, minted another record of the avoidable product-repo denial class that the harvester census counts and nothing retires, and came within one section-read of costing the instruments bucket entirely. The saving the 2026-08-30 section credits the pre-declaration with is still real and still conditional on the declarations agreeing — and the thing that rescued this pass was not a declaration at all, it was a note a previous firing left on this node.

**Limits.** Second instance, same vault, same loop, and the observation cannot be separated from the pass that made it — the non-blinding this node names about itself. The repair remains the one the 2026-09-09 section identified and is unchanged: the withholds block is generated from the run's actual grant, the contradicting sentence is hand-written prompt prose, and deleting three tool names from it is the operator's call on a file outside `product.repos`. Nothing here re-tests `ost_search_web` or `ost_read_web`; neither was called, so the prose's claim about them is still untested and only the `ost_read_repo` disagreement is recorded. Nothing was executed, no rung moved, no instrument set, no status changed, no node created. `ost_check` is withheld on this surface, so this write is unverified by the invariant checker by design.

_Source: this firing's own prompt preamble and call record, first-party._
