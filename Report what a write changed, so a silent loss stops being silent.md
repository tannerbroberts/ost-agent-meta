...
Named in plain text rather than as a wikilink: the test's one backlink belongs to its parent assumption, "Being told a section was dropped is worth little to a caller with no undo".
...
## History
- 2026-08-05 merged "Report what the write changed, so a silent loss stops being silent" into this node and deleted its file — Same solution, same wording — the original was created earlier in this pass with a fabricated provenance id (`TRANSCRIPT:2026-08-05-unattended-pass`) matching no stored evidence record, reported by `ost_check` as an unresolved-citation violation. `source` is frontmatter with no setter on this surface, so re-creating with truthful free-text provenance and folding the original in is the only available repair. The merge carries the original's edge to its assumption across, and the definition-of-done section is restated below with corrected titles.
