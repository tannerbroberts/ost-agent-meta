...
## Sixth trace — 399 calls, 0 failed, the largest clean day on record (unattended sweep, 2026-09-09)

Four lines, extending this node's own trace series rather than opening a subject. The series' last row was 2026-08-19; this closes a three-week gap.

`USAGE:2026-09-09`: **399 invocations across 21 sessions, 399 ok, 0 failed**, p50 104ms, max 5592ms, all on the `mcp` surface. Dispositioned, not mapped, by this node's own standing ruling — **a clean day reveals no need of its own** — the same ruling already applied to `USAGE:2026-07-25` and `USAGE:2026-07-27`.

**The series now reads:** 2026-07-25 108/0 · 2026-07-26 93/62 · 2026-07-27 16/0 · 2026-08-02 240/5 · 2026-08-19 319/1 · 2026-09-09 399/0. The 67% day remains the single outlier this node was distilled from, and the trend since is 2% → 0.3% → 0% at rising volume. This is the largest clean day on record by call count, and the first zero-failure day above 300 calls. On the reading this node has held since its Provenance section — that the 67% was a condition of one day rather than a background rate — that reading is now as well supported as this channel can make it.

**One thing in the same trace that is NOT about failures and is worth a line, because it is the composition rather than the count.** Of the 399 calls, **23 were writes** (21 `ost_append_to_node`, 1 `ost_edit_node`, 1 `ost_annotate`) and 376 were reads (`ost_read_tree` 166, `ost_read_repo` 93, `ost_next_work` 75, `ost_ingest_inbox` 42). Zero `ost_create_node`, zero `ost_set_instrument`, zero `ost_set_status`, zero `ost_link_nodes`. So a 21-session day produced no new nodes, no edges, no instruments and no status changes — every write landed as prose on a node that already existed. Recorded here only because this trace is the artifact that shows it; it bears on the append-accumulation nodes elsewhere on this tree rather than on this node's own claim, and it is left for them rather than argued here.

**What it does not establish.** A zero-failure day bounds nothing about the rate — the four clean days on record sit either side of the one bad one, and this channel counts invocations rather than sampling them. It also cannot see refusals issued outside this product's tool surface, which the "Four more instances" section above establishes is where the larger share of this need now lives; the harness-issued half is invisible to `USAGE:` by construction.

_Provenance: `USAGE:2026-09-09`, read in full this pass — a machine-recorded rollup of the append-only invocation trace, no narrator. Grounds usability, not demand. Disposition and corroboration only; the node's rung is unchanged, no node created, no status changed, nothing executed. `ost_check` is withheld on this surface, so this write is unverified by the invariant checker by design._
