...
## Correction: the backlog is NOT one channel — 18 usage records are unmapped (2026-08-22 unattended sweep, seventh firing)

One short entry per the 2026-08-17 Issues note, and it overturns a standing claim rather than extending the trend.

**Trend first, in one line: 373 → 391.** This pass's `ost_ingest_inbox` captured 3 — two transcript records (`8fb2c38d…`, `ffb9eb47…`) and one usage rollup (`USAGE:2026-08-22`). Both transcript records were read in full and land in rows already held: `8fb2c38d…` is read-before-write (row 1) plus an `overwrite … (y/n [n]) not overwritten` subprocess prompt (the sixth shape, filed 2026-08-22) plus an exit-143 ten-minute timeout (held by "A test that failed because the machine was busy looks exactly like one that failed because I broke something"); `ffb9eb47…` is a denied `Glob` on the product repo (row 2) with `ost_read_repo` live in the same session — the within-session grant split, now reproduced a third time. Nothing mapped, for the reason this node exists.

**The claim that is false.** The 2026-08-21 addendum above states: "Every non-transcript record ever ingested — 73 of 73 — is mapped. 361 of 385 transcript records (94%) are not," and concludes "Nothing else on this channel list contributes a single item to the queue." Counted directly this pass: **`.ost-agent/evidence/` holds 23 `USAGE_` records, and exactly 5 distinct usage ids appear as any node's frontmatter `source:`** — 2026-07-26, -08-02, -08-03, -08-04 and -08-05 (the last cited by four nodes). The other **18 are unmapped**: 2026-07-25, -07-27, and every day from 2026-08-06 through 2026-08-22 except those already named.

Mapped means what the dispose docstring quoted in the entry below says it means — a node's frontmatter `source:` equals the id, body citations being invisible. Neither escape applies here: `agedOutEvidence` was 0 on this pass's sweep and `withheldByDisposition` was `[]`, as the entry below independently records.

**Why the earlier count reached the opposite answer.** It did not count non-transcript records; it inferred them. 458 total minus 385 transcript gave 73 non-transcript, the sweep reported 361 unmapped hence 97 mapped, and 73 + 24 = 97 was read as "all 73 non-transcript, plus 24 transcript." That arithmetic is satisfied equally by 55 non-transcript and 42 transcript, and the entry says so nowhere — it is underdetermined, and this pass's direct count picks the other branch. The addendum's own sort-order argument was sound about `INBOX:` ids and silent about `USAGE:`, which sorts *after* `TRANSCRIPT:` and so hides past the display cap in exactly the way the argument ruled out for INBOX.

**What it changes, and it is not small.** The addendum used the split to relocate the remedy: "the sweep is not where the asymmetry originates: every other adapter produces records that distil into needs and leave, and one adapter produces records that never can." That reasoning is why a fourth candidate — fix it at the transcript adapter, one rolling record per shape — was named as uncovered by the three existing solutions. **The premise is false for the usage channel**, which is a daily mechanical rollup with the same never-drains property and a different adapter. So the asymmetry is not transcript-specific, and a fix built into the transcript adapter alone would leave a second channel accruing. The three queue-side solutions beneath this node look better under the corrected count than they did under the old one.

**The bound, stated so this is not over-read.** This pass counted the `USAGE_` class only, by grep over frontmatter. It did not recount `INBOX_` or `INBOX_friction_`, so "73 of 73" may be wrong by more than 18 and this entry does not claim a full census. One counter-instance falsifies the claim; the true non-transcript unmapped count is still unmeasured.

_Method: `Glob` over `.ost-agent/evidence/USAGE_*.md` (23 files) and an unlimited `Grep` for `^source: '?USAGE:` over the vault, node files and sidecar distinguished by path, diffed by hand. Cross-checked against this pass's own `ost_next_work` for `agedOutEvidence: 0` and `withheldByDisposition: []`. First-party counts over stored records. No node created, no rung moved, nothing mapped._
