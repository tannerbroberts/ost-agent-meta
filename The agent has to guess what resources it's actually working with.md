...
## The two-channel claim, corroborated across the config change that was supposed to fix it (2026-08-31)

Kept short. Only what is new — and what is new is that the 2026-08-28 section's mechanism can now be read off the record set longitudinally, rather than from the single session that noticed it.

**The claim being corroborated.** The 2026-08-28 section argues that the product's source is reachable through two independently granted channels — the harness's own file tools and this product's MCP surface — and that an agent probing "can I read the repo?" is really probing "can I read it *this particular way?*". It establishes that from one firing's own experience: a `Glob` denial followed by a working `ost_read_repo` call.

**What the corpus adds.** Three evidence bodies read in full this pass bracket the change to `product.repos`, and the pairing is what makes them evidence rather than repetition:

- `TRANSCRIPT:14c9afa5-ff0d-46b9-ba9b-c068c08eec63` (2026-08-07) carries **both** refusals in one record: `ost_read_repo` returning "no product repos configured — add local repo paths under `product.repos`", and `Glob` returning "Claude requested permissions to read from /Users/tanner/dev/OST-Agent, but you haven't granted it yet." Two channels onto one resource, both shut, for two unrelated reasons.
- `TRANSCRIPT:03b2fe6f-0338-4243-bcb4-5d908a89514f` (2026-08-06) carries the `Glob` denial twice and nothing else but the prescribed no-argument retries.
- `TRANSCRIPT:f705dfa1-0500-4fdd-acce-5d136a0bbcce` (2026-08-31, captured about an hour before this firing) carries the `Glob` denial as its **only** `tool_error` — while this firing, on the same vault, read `src/`, `test/`, `src/eval/buildable.ts` and this node itself through `ost_read_repo` without a refusal.

**Why the pair is the finding.** `product.repos` was unset on 2026-08-05 (the Issues entry on "A sweep that cannot read its subject reports a clean result" records that refusal verbatim) and is set now. That config change opened the MCP channel and **left the harness channel exactly as it was** — same denial string, same path, 24 days later. The 2026-08-05 note predicted that setting `product.repos` "would unblock instrument-writing across all 64"; it did, for one channel. Nothing about it touched the other, and nothing announced that it hadn't. That is the two-channel mechanism demonstrated by an intervention rather than by introspection, which is the stronger form and the one this node did not previously have.

**The consequence, which is a recurring tax rather than a one-off confusion.** The `Glob`-on-product-path denial now appears in five records on this node's evidence — `03b2fe6f`, `14c9afa5`, `08f7d98f`, `030e5db3` and `f705dfa1` — spanning 2026-08-06 to 2026-08-31. Each is a firing spending a call to discover a grant boundary that no surface states, and each gets back a refusal that is true and licenses a false generalisation.

**The requested fix has not landed, and this is the third pass to say so.** The 2026-08-26 Issues entry and the 2026-08-28 section each ask for the same one-line reconciliation: the firing prompt's hard-rules paragraph says `ost_search_web` / `ost_read_web` / `ost_read_repo` "stay on the attended path", while the skill's mechanically-derived withheld list does not name `ost_read_repo`, and the prompt's own earlier paragraph says all three are on this surface. **This firing's prompt still carries both paragraphs verbatim.** So the contradiction is standing rather than transient, and `f705dfa1` is a firing an hour ago paying for it.
...
## 2026-09-07 — the granted channel is missing a verb, which is a different defect from the grant boundary this node already tracks

Four lines, per this node's convention. Pointer plus one new dimension; the two-channel mechanism is argued in the 2026-08-28 and 2026-08-31 sections and nothing here revises it.

**The instance, first-party to this firing.** Seventh record of the standing signature: `Glob` on `/Users/tanner/dev/OST-Agent` refused with "you haven't granted it yet", `ost_read_repo` then served `test/`, `src/`, `src/ost/instrument.ts`, `src/knowledge/instruments.ts`, `src/eval/buildable.ts`, `src/ost/red-now.ts` and a fixture file without a refusal. Fifth consecutive pass to pay a call rediscovering a grant the skill's withheld list already states correctly; this firing's prompt still carries both contradictory paragraphs verbatim, so the one-line reconciliation asked for on 2026-08-26, 2026-08-28, 2026-08-31 and 2026-09-03 has still not landed.

**The dimension that is new, and it is not about grants.** Every section on this node so far treats the product source as one resource behind two differently-granted channels, and the repair it implies is to state which grant applies. That would not have helped this pass. The channel that *is* granted cannot **search** — `ost_read_repo` lists a directory or returns one file, and has no grep. So a question answerable by one `grep -rn` is unanswerable here regardless of grants, and the failure is a missing verb rather than a closed door.

**What that cost, concretely, because it is a named errand rather than a hypothetical.** The node "Work I already decided needs a person comes back every pass as work I failed to do" carries an errand twice-stated and twice-unclosed: does any CLI entry point print `proseDeclared` — the fact that decides whether reading this vault's 113 prose-declared lanes is a build or a command someone types. A 2026-08-31 pass recorded that `src/cli/index.ts` is 177,250 bytes and truncates, that `Grep` over the product `src/` is refused, and asked whoever holds a shell to run `grep -rn "triageLanes" src/cli/`. This pass tried the one route that read remained: listing `src/cli`, which holds **exactly three files** — `index.ts`, `loop.ts`, `vault-option.ts`. There is no smaller lanes module to read instead, so the errand is not blocked by nobody having looked; it is blocked by the granted channel's shape, and it will stay blocked for every future unattended pass by the same mechanism. That is worth knowing before a sixth pass spends a call on it.

**Why this belongs here rather than beside it.** The founder's framing is resources an operator would declare if asked; the recorded extension is resources the machine sits on; the 2026-08-28 extension is which channel reaches them. This adds: *and what that channel can do once it gets there*. A manifest naming the path and the grant would still have left this pass unable to answer the question, which is the test that separates this from the sections above.

**Not acted on.** No node was created — a solution or opportunity for this is structure the sweep did not ask for, and naming it for a human is the whole of what this section does. Nothing was executed, no rung moved, no instrument set, no status changed, no lane set.

_Source: this firing's own `Glob` denial and its successful `ost_read_repo` calls, plus a first-party listing of `src/cli`. Observed behaviour of this product's tool surface; grounds usability and feasibility, not demand. No test was run and no result is recorded._
