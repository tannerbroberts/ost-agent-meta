...
[[The corrections that would save me most are the ones my own tool issues, and those are the only ones nothing remembers]]
...
Being told the right way to do something fixes it once. The next session starts from the same place, makes the same call, and is refused the same way — so the same correction is spent repeatedly and never accumulates into anything.
...
- 2026-08-07 carried across from "A correction lives only as long as the session it was given in", folded in here on this date: *2026-08-05 evidence: observed → assertion — this node's provenance is the restructure that created it, which supports the floor rung only.* Reinstated verbatim rather than re-performed; it records the folded node's own rung history and is the reason this merge ran in the direction it did.
- 2026-08-07 merged "A correction lives only as long as the session it was given in" into this node and deleted its file — Both state one need: a lesson learned in one session does not survive into the next, so the same correction is paid for repeatedly. The 2026-08-07 pass annotated them as near-duplicates and proposed this merge but could not execute it, because `ost_merge_nodes` needs the survivor's full body and that pass had no way to read one. This pass read both files in full first, so that blocker is gone. DIRECTION DELIBERATELY REVERSED from that annotation's recommendation, and the reason is not preference: the folded node rests on `assertion` with `source: tree-restructure:2026-08-05`, while this node rests on `observed` with a TRANSCRIPT provenance and carries the consolidated four-class census. `ost_set_evidence` caps `observed` on provenance that is itself a recording, so had the census been folded into the other node its rung could not have been restored afterwards by any call on this surface — the merge would have silently demoted the best-evidenced opportunity in this branch. The annotation's stated reason for its direction was that the other node held the three solutions; merging repoints edges either way, so those solutions arrive here regardless and this node stops reading as under-served. The folded node's two sentences and its History line are carried below. Its edge from this node was detached first, so no self-link is unioned in.
