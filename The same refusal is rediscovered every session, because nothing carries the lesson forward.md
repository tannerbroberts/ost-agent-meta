...
- 2026-08-07 Near-duplicate of "A correction lives only as long as the session it was given in" — proposed merge, not executed. Both state that a lesson learned in one session does not survive into the next, so the same failure is paid for repeatedly. The other node already carries three solutions ("Refusals are written back as a standing corrections file every session reads first", "The second identical failure is answered differently from the first", "Refusals the tool can prevent become refusals the tool never issues") and an assumption beneath the first of them; this node carries none, which is why it is reported as under-served on every pass. Ideating three solutions here would restate that subtree, which is the debt merging exists to pay off. Recommended: merge this INTO "A correction lives only as long as the session it was given in", keeping that node's subtree and folding this node's sharper phrasing — the cost is paid per repetition, not once — into the survivor's prose. This pass did not execute it: `ost_merge_nodes` requires the survivor's complete merged body as an argument, and no tool on this surface returns a node's body, so the merged prose could only have been written by discarding wording this pass was never able to read. Corroborating first-party observation from this same session, recorded here because it bears on the merge: the operator-visible corrections list for this workspace already carries a Bash-tool denial that a later call in the same session hit again.
...
- 2026-08-07 unlinked "A correction lives only as long as the session it was given in" — Detaching ahead of folding that child INTO this node. Without it the merge would repoint the child's inbound edge — which is this edge — onto the survivor and leave the survivor wikilinked to itself. Same precaution taken before the 2026-08-06 fold of "Waiting on a slow external check burns the session" and before this pass's fold of "I can't tell what a half-finished run actually finished". The child is briefly orphaned; the merge that follows removes it.
