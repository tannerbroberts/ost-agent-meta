...
## Evidence — queue now 317 records deep (unattended sweep, 2026-08-19)

`ost_next_work` reports 317 unmapped evidence items this pass, up from 290 on 2026-08-18 and 234 on 2026-08-17 — roughly 27-56 new records per day, almost entirely self-generated `TRANSCRIPT:<session-id>` friction records this solution targets. Same decision as the two prior passes: not duplicating them into individual Opportunities. This pass also confirmed directly (via a denied `Glob` on the product repo, see "The agent's repo sight fails mid-pass...") that this unattended surface cannot write or run the instrument above even if it wanted to — building this solution needs an attended session with repo write access, not another discovery pass. The backlog will keep growing at this rate until either this solution ships or a human decides to stop ingesting individual friction transcripts as separately-trackable evidence.
