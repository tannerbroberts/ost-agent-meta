...
**Compared to the alternatives.** The only one of the three that removes the operator's turn entirely in the common case, and the only one that keeps working when nobody is there — which matters because the need it serves sits under work the operator most wants to run unattended. Where the other two make a question cheaper, this makes most questions disappear. It is also by far the most dangerous: it converts the operator's silence into consent, so a run that misjudges which decisions are safe to take this way does damage without ever being told no.
