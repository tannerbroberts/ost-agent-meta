...
## The queue returned 66 → 70, and the four entrants keep the zero — 2026-08-11, unattended sweep (not a recorded result)

**No test was run.** `solutionsMissingInstruments` stood at 70 this firing, up four from the previous firing's 66. The four entrants are the CONVO:2026-08-11 operator-session family: "A controlled opportunity-statement grammar, so wording variance cannot fake or mask overlap", "Every new sibling opportunity states its differentia at creation, or the create is refused", "A deep-dive firing that spends its whole budget thinking about one branch, and clears no buckets", and "Buckets ordered by branch, so consecutive firings finish a region before sampling the next".

**All four were opened this firing, and each rests on one assumption that names a person as the measurement.** The grammar's assumption is whether the operator still recognizes their own need after normalization; the differentia gate's is whether authors under pressure produce real discriminators rather than boilerplate; the deep-dive's is whether its output changes a decision the operator can name; the bucket ordering's is whether the operator reads sweep-chosen order as ordering or as target selection. Their tests already appear in `assumptionWork.needsHumans` ("After three deep-dive firings the operator names a decision their output changed", "Ask the operator, ruleset line in hand, whether sweep-chosen order violates their auto-select rule", and the two operator-judgement studies beside them). Class: `people`, all four. `mechanical` remains **0 of 70**.

**Note the near-miss worth a future builder's attention.** The differentia gate is the most mechanical-looking entry to enter this queue in weeks — a create-time refusal is code behaviour, and a spec could settle whether the guard refuses. But its assumption test does not ask whether the guard refuses; it asks whether the differentiae authors write under the gate discriminate anything, which no exit code holds. If a human splits that test into a feasibility half (guard refuses without a differentia) and the existing desirability half, the feasibility half would be this queue's first genuinely mechanical entry. That split is a structural change to somebody else's node and is left to an attended pass, per the same rule the 2026-08-10 census applied to the split class.
