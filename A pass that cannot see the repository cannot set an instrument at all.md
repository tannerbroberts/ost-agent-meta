...
- 2026-08-26 2026-08-26 unattended sweep, repo sight held — a second and independent mechanism for this node's conclusion, observed by refusal rather than read from source. This pass held full repo sight, had read `src/adapters/transcript.ts` in full, and knew both the existing spec file and the exact case it wanted to assert. It composed the strong form the ruleset itself holds up as the example — `npx vitest run test/adapters/transcript.test.ts -t "a repeated no-argument call with no error files nothing"` — and `ost_create_node` REFUSED it: "contains shell punctuation. Instruments are run as argv with no shell … Name one spec file." So the `-t "<case name>"` filter is not writable from an agent surface at all. The consequence is sharper than not-being-able-to-author-specs: because a bare path to an EXISTING spec file passes today (and a passing instrument is refused too), the only instrument an agent can successfully write is a bare path to a file that does not exist — which is by definition a `no-spec` red. Repo sight does not soften this. A pass with perfect knowledge of the code is funnelled into the same weak red as a blind one; the grammar, not the sight, is the binding constraint. Worth a human's eye because the ruleset text and the tool's accepted grammar currently disagree in a way that reads as agent laziness in the output: this firing's own prompt cites the `-t` form as the strong example to prefer, and the write path rejects it. Two repairs, both a human's call — accept a `-t <name>` argument as a separate argv element (no shell involved, so the stated reason for the refusal does not apply to it), or state in the ruleset that a new spec path is the only writable form and stop asking passes for a red they cannot compose. This sweep set two instruments today and both name new files for exactly this reason; the assertion contracts are written into the test bodies instead.
...
## The config/loop family is a seventh sample, and it pays — two for two (2026-08-22 unattended sweep, seventh firing, repo sight held)

The sixth firing closes on "five families sampled, five hits", then bounds itself to five of six after the harness family failed. The distinction it names as the cheap sort — **the reframe finds a mechanical half when the solution's premise is a claim about *this* product's behaviour, and nothing when the premise is about an external dependency** — was used here as a filter rather than re-derived, and it held on the first family it selected.

**The family.** Config and loop-runtime entries, which no prior firing sampled: every earlier one drew from loop/reporting, unattended-runtime, axiom/goal-acquisition, commercial, or harness. Two entries opened, two mechanical beliefs missing, each now on the tree with a bound threshold and a no-spec instrument.

| Queue entry | Belief that was missing | Why a spec settles it |
|---|---|---|
| Settle the standing answers once, in committed configuration the run inherits | a standing answer written into the config actually reaches the run | `ConfigSchema` declares no key for any settled prompt, and Zod strips undeclared keys rather than rejecting them |
| A deep-dive firing that spends its whole budget thinking about one branch | a firing can be given a budget of its own, and its spend attributed to one branch | `LoopSpendSchema` is a rolling whole-vault window; `UsageEvent` carries no branch field |

**The first is the one that pays for the exercise, and it is a silent-failure finding rather than a missing-feature one.** `ProcessSchema`'s own comment states the mechanism plainly — the schema "uses Zod's default object behaviour, which strips undeclared keys instead of failing" — and argues for it correctly, because it is what lets a vault predating a schema change keep loading. The cost nobody had written down is that the same rule makes *every* forward-looking config key an operator might commit into a no-op with no error. That solution's entire advantage over its siblings is that the run continues rather than stopping, and the advantage rests on a trip from file to run that the product does not make. A pass reading only the solution's prose would find it plausible; the defect is one file away and invisible from the tree.

**The second sharpens what "deliberately unbuilt" means for the deep-dive.** Its mechanism sentence contains two mechanical claims — "its whole budget" and "one branch" — and neither names anything the product has. `test/loop/spend-ceiling.test.ts` specs the ceiling as a rolling window stamped at `loop start`, explicitly proof against mid-firing widening; there is no per-firing allotment to give a deep-dive. And `UsageEvent`'s field list is closed at `ts, tool, ok, ms, surface, argBytes, err, denied, session, unknown, wrote` — `surface` reads `mcp` for every firing type, and `wrote` is empty for a firing that only appends, which is the firing type proposed. Recorded on the node with the honest qualification that branch attribution *is* recoverable by hand from commit messages, and that recovering it by hand spends the one resource this tree treats as scarce.

**The bound, in the form the previous four firings used.** Two entries is a sample, not a recount. The standing `mechanical = 0` finding is untouched: both solutions still carry a genuine human ask — whether pre-answering is wise, and whether analysis-only firings change any decision — and neither became buildable. What generalises is the accounting, now across **six families of seven sampled**, with the single failure being the one the sixth firing's filter predicts.

**One thing this firing did NOT do, deliberately.** It considered "Name the specific mechanisms a hand process structurally cannot have" and put it down: its unwritten belief — that each named mechanism is bound to code whose removal reddens — is the belief the sixth firing already wrote beneath "Lead with what this refuses to do". Writing it a second time would have produced a hit for the tally and a duplicate for the tree, which is the defect this node's chain exists to catch.

**A separate finding about the sweep instruction itself, worth a human's eye.** The unattended prompt's worked example of a strong instrument is `vitest run test/git/conflict-guard.test.ts -t "refuses a write whose base hash drifted"`. That form is not expressible: `INSTRUMENT_FORMS` is anchored on the filename and `SHELL_METACHARACTERS` refuses the quotes. The 2026-08-21 entry above records the same refusal empirically. So a sweep following the instruction literally cannot comply, and the instruction should either drop the `-t` example or the grammar should gain a form for it — a product decision, named here rather than acted on.

_Method: `ost_read_repo` reads of `src/config/schema.ts`, `src/telemetry/usage.ts`, `src/knowledge/instruments.ts`, `src/ost/instrument.ts`, `src/mcp/server.ts` and `test/loop/spend-ceiling.test.ts` (all `truncated: false` except `schema.ts`, read to the `defaultConfigYaml` scaffold), plus listings of `src/`, `src/config/`, `src/ost/`, `src/mcp/`, `test/`, `test/config/` and `test/loop/` to confirm both named spec paths are absent. Nothing executed. Grounds feasibility only; silent on whether anyone wants either solution. Rung stays at the `assertion` floor._

## Addendum, same firing: an eighth family — onboarding/init, one for one

Short addendum to the entry above rather than a new section, because it adds a sample and no new argument. After the two config/loop entries left the queue (67 → 65, both absent from the next listing, two previously-hidden entries surfacing to replace them — the sixth firing's self-scoring observation reproduced independently), this firing took one of the newly-surfaced entries: **"Ship a starter vault whose outcome is a placeholder the human must replace"**, from the onboarding/init family, which no prior firing has sampled.

| Queue entry | Belief that was missing | Why a spec settles it |
|---|---|---|
| Ship a starter vault whose outcome is a placeholder | a node written under a placeholder root is distinguishable afterwards from one written under the real mandate | `OstNode`'s field list carries no mandate generation, and `serialize` drops any unenumerated key |

**Why this one is worth the tally rather than just filling it.** The missing belief is not adjacent to the solution's stated risk — it *is* the solution's stated risk, in the half nobody had written. Its prose already argues against itself correctly: "every node created before it is replaced ladders up to a goal nobody chose." What the prose does not say is that the damage would be **unidentifiable**, because nothing on a node records which mandate was in force. So the sequencing warning on that node ("cheapest to build here and the most expensive to be wrong about") understates its own case: being wrong would leave no trace to find.

**The qualification, carried onto the assumption rather than hidden here.** A date-based partition against the root's `## History` recovers part of it — but `created` is `YYYY-MM-DD` with no time, so it degrades to nothing exactly in the success case (a placeholder replaced the day it was scaffolded) and works only in the failure case (one that sat for weeks). The test is constructed same-day on purpose so a builder cannot satisfy it by reading dates.

**Running tally, in the sixth firing's form.** Seven families sampled across five firings, six hits, one structural failure — the harness family, where the premise is a claim about an external dependency and the `humansRequired` framing is correct rather than unrecounted. The sixth firing's sorting rule (does the premise claim something about *this* product?) predicted every result including its own exception, and was used here as a filter rather than re-derived.

_Method: `Read` of the solution node, plus first-party `ost_read_repo` of `src/ost/node.ts` and `src/ost/frontmatter.ts` in full and a listing of `test/ost/` (47 specs, none reading mandate provenance). `src/cli/index.ts` deliberately not read — it exceeds this surface's cap, so the solution's own claim that `set-outcome` preserves the prior mandate in the root's history is quoted and left unverified rather than guessed. Nothing executed. Rung stays at the `assertion` floor._
