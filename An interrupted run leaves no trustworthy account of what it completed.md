...
## Corroborating evidence read and skipped — 2026-08-10 (names an id, discharges nothing)

`INBOX:2026-07-24-friction-a-backgrounded-session-leaves-no-marker-of-where.md` — filed by the builder-loop at the moment of friction: "An earlier builder pass was backgrounded mid-work; the next pass had no way to tell what it had finished versus abandoned." This is a second first-party instance of this node's need, not a new need, so no node was minted from it. It appears in the legacy `mapped.json` list, so it was discharged once and re-entered the unmapped queue when the mapping relation became derived from node `source:` frontmatter — the reopening itself is the need recorded on "An upgrade changed what counts as done, so finished work silently reopened". Named here in plain text because frontmatter citations are matched exactly and this node's `source:` already carries its own id; the record therefore stays in the unmapped count until a class-level discharge mechanism ships.
