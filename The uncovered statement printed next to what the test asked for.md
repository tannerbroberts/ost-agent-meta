...
- 2026-08-21 status: shipped → shipped — Promotion to shipped recorded with reasoning so trustsShippedStatus (src/eval/shipped-audit.ts) stops reporting this node as instrument debt; its History recorded no promotion line. Verified against the repository this pass via ost_read_repo: src/eval/coverage.ts exports computeCoveragePairs(), which pairs every bounded AssumptionTest's pre-committed threshold (askedOf, reading the `threshold` field and falling back to the bold pre-commit lead-in) with its ## Uncovered statements, and reports `asked: null` for a bounded test that never wrote a threshold down — the two behaviours this node claims for v0.9.0. The CLI print path in src/cli/index.ts was not read (file too large for this surface), so what is verified is the module, not the rendered line. Read, not run: no spec executed, no result recorded, not a validation.
