...
## 2026-09-07 — the fixture question left open on 2026-09-02 is closed, and the remaining gap is now located exactly

Four lines, per this node's convention. Only what is new, and it closes a named errand rather than re-arguing the grammar.

**The errand.** The 2026-09-02 section ends: "Whether the second spelling is covered by a case somewhere else — the fixtures directory the comment names is the obvious candidate — was not verified, and this pass makes no claim either way." Read first-party this pass: `test/fixtures/instrument-red-now/PROVENANCE.md`, in full.

**Answer: yes, it is captured.** The directory holds seven recorded runs, and `empty-spec` is exactly the second spelling — a spec file with no test case, against which vitest emits `No test suite found in file <path>`, *not* "No test files found". PROVENANCE.md states the consequence in this node's own terms: `collectedNothing` matched only the first string, "so an existing spec with no test case in it was classified `red` — a file that cannot fail, minting a permit — while the comment above that function said the empty-spec case was covered. It was not, until this capture." Every entry is a real `spawnSync` capture from 2026-08-31 against `vitest@2.1.9`, npm 11.x, macOS — not strings anyone imagined, which is the file's own stated reason for existing.

**The half that is still open is now a smaller and better-aimed question.** None of the seven entries — `assertion-fails`, `unbuilt-local-module`, `empty-spec`, `missing-package`, `passes`, `runner-absent`, `spawn-failed` — is a `-t` filter matching nothing inside a file that exists and collects. So the wording a title-filter repair would have to match remains the one unobserved fact in the chain, exactly as the 2026-08-31 (later firing) section said, and it is now known to be absent from the one place it would have been recorded rather than merely unconfirmed there. Whoever takes route one inherits a worked capture procedure and one capture left to run.

**One incidental finding from the same file, worth a line because it bears on every instrument this tree records.** PROVENANCE.md notes that a first attempt at the `runner-absent` capture on a networked box had npx silently download `vitest@4.1.11` and run the spec under it. So an instrument executed in a checkout that has not installed its own dependencies is not measuring that checkout's suite at all — a green or red from such a run is about whatever npx fetched. That is a distinct failure mode from the four `Observation` classes and nothing in the classifier detects it.

**Not acted on.** No instrument was set, no threshold changed, no lane set, no rung moved, no status changed, no node created. The missing `ost_set_threshold` sibling, the three-readers-of-`thresholdKindOf` disagreement, and the open ask about whether that omission is deliberate all stand unchanged.

_Method: first-party `ost_read_repo` of `test/fixtures/instrument-red-now/PROVENANCE.md` in full and a listing of that directory, plus `src/ost/instrument.ts` and `src/knowledge/instruments.ts` in full. Read off the source and the recorded captures; nothing executed. Grounds feasibility only; rung unchanged at the floor._
