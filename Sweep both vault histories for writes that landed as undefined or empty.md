...
- 2026-08-31 2026-08-31 unattended sweep — recording why this node keeps surfacing as owing an instrument and cannot honestly be given one. Its Issues section records the sweep RUN on 2026-07-27 across all 106 commits of both vaults, 306 annotation entries classified, with the pre-committed threshold explicitly evaluated: 21 literally `undefined`, 0 empty/whitespace, 0 truncated — zero of any other shape, so the assumption survived, and the consequence was taken (v0.18.0 shipped the guard as a tripwire for one known shape rather than as the primary fix). The question is answered; the answer lives in prose rather than in a `## Results` block, which is the only reason any analysis still counts it outstanding. A command asserting the shipped guard would pass on arrival, and the ruleset forbids an instrument that cannot fail. The node also declares **Lane: compute-only** in its own prose while carrying no `lane:` field, so the fail-closed lane reader counts it as needing a person while its text says compute may run it. For a human, either drains it: `ost-agent result` to record the verdict, or `ost-agent lane --set` to make the declared lane a field. Neither is on the unattended surface. The caveat the node named before it ran still stands and is not cleared: nothing here can see a write that was well-formed and WRONG, which remains the alarming shape. Counted as one of four of this shape under "Work I already finished keeps coming back in the queue, so the pass can never say it is done". No instrument set, no rung moved, nothing re-run.
