...
## 2026-09-07 — the rate collapsed, and the conclusion recorded on 2026-09-03 does not survive its own next data point

Kept short, per this node's convention. One number, and it goes the other way from the last three.

**The series, extended over a four-day gap rather than a same-day one.** The 2026-09-03 section recorded the queue reaching **609** and concluded, of the +33/day it measured against the +14 and +4 before it, that "the corrected rate holds and the morning figure remains the outlier." This firing's stop condition, computed before it ingested anything, read **613**; `ost_ingest_inbox` captured exactly **1**, reaching **614**.

**So: +5 across four days — about 1.25 a day.** That is below every figure previously recorded on this node, including the +4/day the 2026-09-02 morning firing measured and that two later firings dismissed as an outlier. Had +33/day held, the queue would stand near 740 today. It stands at 614.

**Which of the 2026-09-03 claims survive.** Its measurement stands — the queue really did move ~+33 in that interval. Its *conclusion* does not: "the corrected rate holds" was a claim about the generator, made from three points inside a 48-hour window, and the first point outside that window is a quarter of the smallest of them. The honest reading now is that this queue's arrival rate is **bursty and tracks how often the loop fires**, not a rate at all — which is precisely what this node's own structural inference predicts ("arrivals are a lower bound on firings"), and four quiet days is the same inference running in reverse. The 2026-09-02 morning firing's stated limit — "two snapshots is not a trend" — turns out to have been the soundest sentence in the series, and it was written by the firing whose number was later called wrong.

**What this does to the drain, which is still the larger fact.** **614 items, 0 retired.** With arrivals at roughly one a day, the backlog is now almost entirely historical: essentially none of the 614 is recent. That sharpens the harvest-side-suppression argument in the opposite direction from the one this node has been reasoning in — suppressing new records saves ~1 record a day against a standing 614, so suppression is close to irrelevant to this queue's size, and only retirement of the existing stock can move it.

**Composition of what did arrive, read in full.** Two bodies: `030e5db3` (9 events — a `Glob` permission refusal, three "requested permissions to use `mcp__…`" refusals for `ost_flag_humans_required`, `ost_check` and `ost_status`, and five no-argument retries of the prescribed `ost_ingest_inbox`/`ost_next_work` pair) and `f5883198` (2 events, both no-argument retries, the pure self-manufactured shape). Every event in both is on the eight-row signature table. **New needs distilled: zero** — the sixth consecutive pass to record that, and deliberately no Opportunity was created, for the reason this node has stated each time: `source:` is a single field, so "mapping" these would mint duplicates whose only function is to decrement a counter.

**One forward observation, offered as a single data point and not as a finding.** `030e5db3` spends three calls discovering withheld tools by invoking them — the concentrated signature this node singles out (91 events across 29 records). This firing spent **zero** on that, because its prompt named the withheld set up front. One session against one session, no controlled comparison, and the two firings differ in more than that; it is recorded here only because it is the first instance in this corpus of that signature being absent where the surface had changed.

_Method: this firing's own stop-condition figure and `ost_ingest_inbox` output, plus two evidence bodies via `ost_next_work({evidence})`. Observed behaviour of this product's own agent, captured mechanically — it grounds usability, not demand. The records stay listed as unmapped; reading them does not map them. Nothing executed, no rung moved, no instrument set, no status changed, no node created._

## 2026-09-08 — the largest single-day arrival yet recorded, and it withdraws yesterday's "suppression is irrelevant" conclusion

Kept short, per this node's convention. One number, and it corrects a conclusion drawn yesterday rather than adding a census. It deliberately does **not** restate the zero-new-needs finding, which six consecutive sections already carry.

**The series, extended.** The 2026-09-07 section recorded the queue at **614**, measured **+5 across four days (~1.25/day)**, and concluded that the rate had collapsed. This firing's stop condition, computed before it ingested anything, read **641**; `ost_ingest_inbox` captured 1, reaching **642**.

**So: +28 in roughly one day** — the largest single-day movement recorded on this node, above the +33-over-a-longer-interval and more than twenty times yesterday's figure.

**What survives.** The 2026-09-07 *reframing* stands and is strengthened: this queue's arrivals are firing-driven and bursty, not a rate at all. At least four firings are recorded against 2026-09-08 across this tree's nodes — two on "An operator-set evidence window in ost.config.yaml, amended by hand like discovery.target", one on "My instruments are red because a file is absent, not because the behaviour is", and this one — against four quiet days before it. That is the node's own structural inference (arrivals are a lower bound on firings) running forward instead of backward.

**What does not survive, and it is the actionable half.** That section concluded from the quiet: "the backlog is now almost entirely historical: essentially none of the 614 is recent… suppressing new records saves ~1 record a day against a standing 614, so suppression is close to irrelevant to this queue's size, and only retirement of the existing stock can move it." On a busy day suppression saves ~28, which is roughly **4.5% of the standing queue per day**. So suppression's value is not small and not fixed — it is **proportional to how often the loop fires**, which is the one quantity in this system the operator sets directly (`loop.cadence`, currently `1h`). A reader weighing harvest-side suppression against stock retirement should price it against the busy day, not the quiet one, because the busy day is the configured default.

**Limits, stated so this is not over-read.** Two points a day apart, which is the same weakness the 2026-09-02 morning section named and was later corrected on; this one is offered as a third swing of a series that has now gone +4, +14, +33, +1.25, +28, and the honest summary of that series is variance, not trend. The 641 is this firing's stop condition and the 614 comes from another firing's prose rather than a timestamped ledger, so neither reading fixes an hour. The four-firings count is drawn from dated sections on three nodes, which is a floor on firings and not a census of them — a firing that wrote nothing would be invisible to it. Nothing was executed, no rung moved, no instrument set, no status changed, no node created.

_Method: this firing's own stop-condition figure and `ost_ingest_inbox` output, plus dated sections read via `ost_read_tree` on three nodes. Observed behaviour of this product's own agent, captured mechanically — it grounds usability, not demand. The records stay listed as unmapped; counting them does not map them. `ost_check` is withheld on this surface, so this write is unverified by the invariant checker by design._
