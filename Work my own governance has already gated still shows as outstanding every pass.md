...
## A fourth measurement, and the first from outside the capped window (unattended firing, 2026-08-28)

The three measurements above are explicitly bounded: "Windowed samples; the unlisted entries are untriaged and the `mechanical = 0` claim is not extended to them." Every census of this bucket — 0 of 33, 0 of 25, 0 of 5 — was taken over the same frozen head, which the sibling node "Most of my backlog has never been read once, because every firing is shown the same alphabetical first 25" establishes never rotates. So the strongest claim on this node rested on a sample that structurally could not reach the 39 entries sorting after "Short-lived scoped tokens…". This firing reached one.

**How, since the sweep will not name them.** `ost_next_work` serves only the 25 titles it shows. But a solution's title is its filename, so `Grep` for `^type: Solution` over `[S-Z]*.md` in the vault returned 80 node files in the invisible range, and `ost_read_tree` reads any of them by title. The tail is addressable today, by any pass willing to spend two calls — no rotation fix required.

**The entry drawn, and it was not drawn at random.** "The instrument-writing step declares repo sight required and skips itself rather than inventing paths" was chosen because it is the S–Z entry most likely to be *mechanical*, and therefore the one most likely to falsify the zero. It describes a concrete change to a concrete step, of exactly the shape a spec file usually settles.

**It is a decline, on the same grounds as the head.** The node carries its own `## Definition of done` reading: *"This one has no command, deliberately. Whether an instrument is grounded is a property of what its assertions mean against a codebase, and a missing-file red and a broken-mechanism red both exit non-zero today — which is exactly why the distinction has to be read rather than run."* It fixes a bar (the proxy must agree with a blind reader on at least 8 of 10), names its own counter-evidence, and instructs that it not be built before that test runs. A pass writing an instrument here would be overwriting a reasoned refusal, not clearing debt.

**What that does and does not settle.** One entry is not a sample of thirty-nine, and it was chosen adversarially rather than randomly — which cuts in the finding's favour, since the entry picked *to break* the zero did not break it. The narrow, honest claim: the unlisted region is not visibly different in kind from the listed one, and the first probe into it behaved exactly as the three windowed censuses predicted. The bound above should be relaxed, not removed — `mechanical = 0` now has one data point outside the window rather than none, where before it had zero.

**A second thing this entry says, worth more than its triage verdict.** Its premise is this firing, described two weeks in advance. It was ideated by the 2026-08-06 pass, which had no repo sight, and its argument is that an unattended loop "asked every pass for 64 instruments will produce those in volume — and each one *removes* its solution from `solutionsMissingInstruments`, so the bucket reports progress while the tree fills with commands that measure nothing." This firing began by concluding it had no repo sight — its `Glob` probe on the product path was genuinely denied — and was heading toward exactly that outcome before finding that `ost_read_repo` was granted after all (recorded on "The agent has to guess what resources it's actually working with"). The candidate is a live description of a failure mode this vault reproduces, which is an argument for a human ranking it rather than for a sweep instrumenting it.

_First-party to this firing, 2026-08-28: `Grep` over the vault's node files, `ost_read_tree` on the named solution. Read off node prose; nothing was executed, no result is recorded, and no rung moves._

## A fifth measurement narrows the zero: "0 mechanical" should read "0 instrumentable today" (unattended firing, 2026-08-28, second of the day)

The four measurements above all report the same finding — no mechanical work in this bucket — and the fourth relaxed its bound by reaching one entry outside the capped window. This firing reached a second one, and it does not fit the category.

**The entry.** "State timing gates as work completed rather than wall-clock, so a busy machine cannot fail one", drawn from the hidden tail by the same two-call method the section above describes. Its sole test is "A load-independent gate keeps its verdict when the machine is saturated". Its stated definition of done names `npx vitest run test/gate/operation-budget.test.ts`, and `ost_read_repo` confirms no `test/gate/` directory exists — so the command as written would be a `no-spec` red and was correctly not set.

**Why it is nevertheless not a `people` entry.** Every prior decline in this bucket resolved to a belief no exit code can reach — whether a person keeps a file current, whether strangers recognise their own discipline, whether an operator amends a config by hand. This one does not. Read first-party this pass, both halves of the measurement are already committed code: `test/telemetry/work-units-vs-elapsed.test.ts` is a green instrument that defines a load-independent work unit and asserts it is byte-identical across repeated runs of an unchanged fixture, and `test/perf/contention.ts` exports `spinners(count)` — forked processes that pin cores until killed — with a docstring saying it exists precisely so that tests timing work on a deliberately busy machine do not each reinvent it. Nothing combines them. The question "does the gate keep its verdict under saturation" is answerable by one spec composing two committed helpers, and by nobody's afternoon.

**So the classification the prior censuses used is missing a cell.** They sorted entries into `mechanical` / `people` / `elapsed-time` / `shipped` / `split` and reported `mechanical = 0`. This entry is mechanical in kind and un-instrumentable in fact, because the honest instrument names a spec that has not been written yet — and the ruleset forbids reserving the path in advance. That is a different state from "a person must answer this", and it points at a different remedy: not a lane label, but a builder writing one spec file.

**What this does and does not change.** It does not overturn the labelling-backlog finding: the head of the bucket really is dominated by human-only beliefs, three censuses agree, and the ask for one attended labelling pass stands unchanged. It narrows the strongest sentence on this node. "The residue of this bucket is a labelling backlog, not a build backlog" is now true of most of it rather than all of it, and the accurate form of the zero is **0 instrumentable today**, not 0 mechanical. Whoever runs the labelling pass should expect to route a small number of entries to a builder rather than to a lane.

**Bound.** Two entries now sampled from the tail of thirty-nine, neither at random — the fourth measurement picked the entry most likely to be mechanical and it declined; this one picked from the same region and it did not. Two adversarially-chosen points do not estimate a rate, and no claim about the other thirty-seven is made here.

_First-party to this firing, 2026-08-28: `ost_read_repo` reads of `test/telemetry/work-units-vs-elapsed.test.ts` and `test/perf/contention.ts` in full and a directory listing of `test/`, plus `ost_read_tree` on the named solution. Nothing was executed, no result is recorded, and no rung moves._
