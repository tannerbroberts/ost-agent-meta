...
## 2026-09-03 — the guard that would refuse the weak form is built, is off in this vault, and would not refuse it anyway

Kept short, per this branch's convention. Three first-party facts, then one consequence that composes them; the consequence is the new part.

**A write-time guard exists, and no section above mentions it.** `src/ost/red-now.ts` runs a candidate instrument *before accepting it* and sorts the run three ways: `accepted` (a spec was collected and an assertion in it failed), `already-built` (it passed — refused), and `declined` (nothing was measured — refused, with `no-spec` and `unavailable` deliberately kept apart because one says write the spec and the other says fix the box). This node already records the `no-spec` distinction at verify time (2026-08-09) and at spend time through `confirmPermit`. The boundary that refuses a command *on the way in* is a third site, and it was not on this node.

**It is off here.** `instruments.runOnWrite` defaults to false. That default is not inferred — it is pinned by `test/mcp/instrument-red-now-guard.test.ts`, whose case "without the operator's grant, a green command lands unchallenged" exists precisely to keep the cost of the default visible. This vault's `ost.config.yaml` carries no `instruments:` block at all, so the grant is absent. Every one of this tree's 377 instrument fields was therefore accepted on its author's word, on the surface whose whole design premise is that an agent's unchecked claim is worth nothing.

**And turning it on would not close the weak form.** The same spec pins the waiver: "a bound threshold still carries a spec nobody has written" — a `no-spec` command lands anyway when the test carries a bound `threshold:`. Only two things would newly refuse: a command that passes on arrival, which gets no waiver ("a bound threshold does not carry a command that PASSES"), and a `no-spec` command on a test with no fixed bar.

**The consequence, and it is why this bucket is structurally unclearable rather than neglected.** Compose that with this node's 2026-08-21 finding that the accepted grammar is a bare spec path with no `-t` filter. The unit the grammar can name is a *file*; the unit that can be red about behaviour is an *assertion*. Against a suite kept green there is therefore no third case: a named spec either passes today — refused as `already-built` — or does not exist, which is `no-spec` and lands only under the threshold waiver. The strong instrument this node was created to ask for requires naming a whole spec file that fails today, and a whole spec file that fails today in a maintained suite is a broken build, not a discovery instrument. With the guard on, the only instrument writable on this surface would be a missing-spec path on a test that already carries a bound threshold.

That is the same place the 2026-08-21 section arrived at from the tool boundary, reached instead from the config and the guard, and it sharpens the claim: the problem is not that this surface writes the weak form badly, it is that the weak form is the only form a file-granular instrument can take against a passing suite.

**What this does not settle.** Whether `runOnWrite` should be turned on. It would still catch green-on-arrival commands, and the tree's 260 green instrument observations suggest that is not a hypothetical class, so there is a real gain on the table — this section argues only that it would not clear the weak-red backlog, not that it is worthless. It also says nothing about whether the waiver is right, nothing about desirability, viability or usability, and nothing about the 70 solutions in the instrument bucket beyond why pointing commands at them would not help.

**Limits.** The fourth paragraph is an inference composed from read code and pinned specs, not an observation. No `ost_set_instrument` call was attempted this pass, and with the grant absent none would have exercised the guard in any case. "A suite kept green" is an assumption about the product's trunk that this pass did not verify, and verifying it means running the suite, which is not on this surface. The config claim rests on this vault's `ost.config.yaml` alone; a grant supplied from somewhere other than that file would not have been seen.

_Method: `ost_read_repo` reads of `src/ost/red-now.ts`, `src/ost/instrument.ts`, `test/eval/vacuous-red.test.ts` and `test/mcp/instrument-red-now-guard.test.ts` in full, plus a read of this vault's own `ost.config.yaml` from disk. Committed code and committed specs, read first-party; grounds feasibility, not demand. Nothing executed, no instrument set, no rung moved, no status changed, no node created. `ost_check` is withheld on this surface, so this write is unverified by the invariant checker by design._

## 2026-09-08 — the documented mitigation was not taken, by a pass that had every reason to take it

Kept short, per this branch's convention. One new fact, and it is about the mitigation rather than about the grammar.

**What this pass did.** Working `solutionsMissingInstruments`, it found "Every door that puts a test in a needs-a-person lane files an ask on the way through" — a solution whose Definition of done already carries a written command and whose `instrument:` field is empty, which is the one shape in that bucket that looks like genuinely un-done work rather than a recorded decision. It read the target spec first (`test/ost/pending-ask-queue.test.ts`, 6,829 bytes, in full) and confirmed the claim the node makes about it: the file exists, collects four passing cases across three describe blocks, and its `beforeEach` creates the AssumptionTest with `instrument:` only, so no case anywhere creates one through the `humansRequired:` argument. Then it called `ost_set_instrument` with the `-t` filter and was refused: *"contains shell punctuation… Name one spec file."*

**Which is the fourth boundary observation, and not the new part.** The 2026-08-21 section records this refusal verbatim, the 2026-09-01 section re-verified the validator by reading it, and the 2026-09-02 section names `ost_create_node` refused on **this same test** — "A test born humans-required at creation is in the queue with a non-null age" — for the same `-t` construction on 2026-09-01. So one test node has now drawn the identical refusal twice, eight days apart, from two passes, through two different tools.

**The new part: the mitigation this node recorded does not survive contact.** The 2026-09-01 section ends: *"This pass avoided that only by reading the validator first and declining to make the call, which is not something the instruction suggests."* This pass did not avoid it. It read the spec file it was pointing at — the diligent step, and the one the prompt does ask for — and still walked into the refusal, because nothing connects "check the spec exists" to "check the command is expressible." `src/knowledge/instruments.ts` is not on the path of a pass doing instrument work; it is on the path of a pass investigating why instrument work fails. So the recorded workaround is available only to a pass that has already been burned, which means it is not a mitigation at all. That is a stronger statement than the earlier section's, and it is evidence from the failing side rather than the avoiding side.

**A consequence worth naming for whoever picks up the bucket.** The solution above is, as far as this pass could tell, the closest thing in `solutionsMissingInstruments` to un-done work — and its written command is unsettable. Its `instrument:` field is empty not because nobody wrote a command but because the command that answers it cannot be expressed. A builder or a future pass should not read that empty field as an omission, and should not replace the command with the bare spec path: `npx vitest run test/ost/pending-ask-queue.test.ts` passes today, so it would be refused as already-built, and if it were somehow recorded it would assert a definition of done that is already met.

**What this does not settle.** Nothing about whether `-t` should be admitted — that trade is unchanged and is still a human's, and this pass has no view on it. It does not re-measure the absent-path counts, does not touch the 74 bucket entries, and says nothing about desirability, viability or usability.

**Limits.** One refusal from one pass; the claim that the diligent path does not lead through the validator is an argument about this pass's own reasoning, which is the least independent evidence available for it. The refusal itself is machine-recorded and will be harvested, so it manufactures one more queue record exactly as the 2026-09-02 section predicts — this pass is an instance of that cycle, not an observer of it. Nothing was executed, no rung moved, no instrument set, no status changed, no node created. `ost_check` is withheld on this surface, so this write is unverified by the invariant checker by design.

_Method: `ost_read_repo` reads of `test/ost/pending-ask-queue.test.ts` and `src/knowledge/instruments.ts` in full, plus one refused `ost_set_instrument` call made by this pass and its verbatim message. First-party observation of the product's own tool boundary; grounds feasibility, not demand._

## 2026-09-08 (later firing) — the workaround does survive contact, by a route that does not require having been burned

Kept short, per this branch's convention. One fact, and it corrects the section immediately above rather than adding a fifth boundary observation.

**The claim being corrected.** That section concludes that the mitigation this node recorded is not a mitigation: "nothing connects 'check the spec exists' to 'check the command is expressible.' `src/knowledge/instruments.ts` is not on the path of a pass doing instrument work; it is on the path of a pass investigating why instrument work fails. So the recorded workaround is available only to a pass that has already been burned." That was drawn from one pass that read its target spec, made the call anyway, and was refused.

**This pass is a counter-instance, and the route is the point.** Working the same bucket, it read `src/knowledge/instruments.ts` in full **before composing any `ost_set_instrument` call**, confirmed that `INSTRUMENT_FORMS` holds one anchored pattern with no test-name filter and that `SHELL_METACHARACTERS` refuses both quote characters, and made no call. It had not been burned in this session. So the validator is reachable from the path of a pass doing instrument work, and the earlier section's generalisation is too strong.

**What actually put it on that path, stated so it is reusable rather than lucky.** Not curiosity about failure. The bucket's own branch carries the finding in prose — the census on "Work I already decided needs a person comes back every pass as work I failed to do" states that "the accepted instrument grammar names a whole spec **file** with no test-name filter" — and this pass treated that as a claim to check against the source rather than as a conclusion to accept. The general form: **the tree already holds the refusal, so a pass working a bucket should read the branch's recorded findings before composing a call, and verify the load-bearing one against the code.** That is one read, it is available to any firing, and it does not require the refusal to have happened first. The correction this node's own theme names — that the corrections which would save the most are the ones the product itself issues — applies here in the cheap direction: the correction was already written down.

**The consequence, which is the small measurable one.** The 2026-09-02 section establishes a cycle: refusal becomes a `tool_error`, the harvester files it, the record joins `unmappedEvidence` where it cannot be retired. This firing broke that cycle once, deliberately: no refused instrument call was made, so no friction record was manufactured on this account. One record is not a rate, and the cycle is unchanged for any pass that does not take the route above.

**What this does not settle.** Nothing about whether `-t` should be admitted — unchanged, and still a human's call between widening the grammar and fixing the prompt. It does not weaken the underlying finding: the strong form remains unexpressible, this firing's own instructions still hold up the refused string as their worked example of quality, and the prompt/validator contradiction is verbatim what the 2026-09-01 section recorded. What it changes is only the pessimism about the workaround, and therefore slightly lowers the urgency of the grammar-side repair relative to the prompt-side one, which was already the free option.

**Limits.** n=1 in the favourable direction, and it is this pass reporting on its own reasoning — the least independent evidence available for a claim about what a pass would do, and the same weakness the section it corrects names about itself. A pass that reads the branch first still spends the reads; the route is cheaper than a refusal, not free. Nothing was executed, no instrument was set, no rung moved, no status changed, no node created. `ost_check` is withheld on this surface, so this write is unverified by the invariant checker by design.

_Method: `ost_read_repo` reads of `src/knowledge/instruments.ts` and `src/ost/red-now.ts` in full, plus `ost_read_tree` reads of five nodes in the instruments bucket and its owning branch. First-party read of committed code and of this vault's own nodes; grounds feasibility, not demand._
