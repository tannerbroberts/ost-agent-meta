...
## 2026-09-02 (later firing) — the repeat detector has a blind spot in the opposite direction, and the 197-record figure inherits it

Four lines, per this node's convention. Only what is new, and it qualifies this node's own census rather than adding one.

**What this node establishes about the detector.** Every count here turns on the signature rule — `` `${name}:${JSON.stringify(input)}` `` — and this node uses it in one direction only: the no-argument prescribed pair always collides, so procedure always registers as a repeat. The 2026-08-31 corpus count then measures the must-not-suppress class as **197 of 534 records (36.9%)** carrying at least one argument-bearing retry.

**The instance that shows the other direction.** `TRANSCRIPT:60eba32c-4538-45ca-9445-1731707536ae`, captured at this pass's own ingest, is `9 friction events (tool_error ×9)` — **zero retries**, six of the nine being `await: gave up after 300s` expiries. Since `seenCalls` spans every `tool_use` block in the session and ignores outcomes, zero retries means no two calls shared an input: the six awaits ran six distinct condition strings. The session repeated one question — *has the suite finished yet* — six times over 1800s and registered as a repeat exactly zero times, because the caller incremented its output filename between attempts. The finding and its consequence for that branch's candidate are recorded on "A repeated wait on the same condition resumes and doubles its budget automatically, up to a ceiling set once by hand"; only the detector fact belongs here.

**What that does to this node's numbers, in both directions.** It does not move the 14.2% self-manufactured figure, which rests on collisions that do happen. It qualifies the 197: that count is of records where the detector *fired*, and a semantic repeat with a varying argument is invisible to it, so 197 is a floor on the must-not-suppress class rather than a measurement of it. The direction of the error is the safe one for this candidate — a suppression rule scoped to the no-argument condition cannot take an invisible repeat with it, because an invisible repeat never becomes a `retry` event to suppress. What it does affect is any future work that reads the retry class as a measure of how often the agent repeats itself: on this record the detector's recall is 0 of 6.

**Limits.** One record, and the distinctness is inferred from the absence of retries plus the signature rule read off `src/adapters/transcript.ts`, not from six condition strings observed directly — the expiry lines do not carry their own commands. No claim is made about how often varying-argument repeats occur across the corpus; this establishes that the class is non-empty and that the 197 is a floor, nothing more. Nothing was executed, no rung moved, no instrument set, no status changed, and this node's humans-required Definition of done stands unchanged.

_Source: this pass's own `ost_ingest_inbox` output, one evidence body via `ost_next_work({evidence})`, and a first-party `ost_read_repo` read of `src/adapters/transcript.ts` in full. Observed behaviour of this product's own agent — it grounds usability, not desirability._
