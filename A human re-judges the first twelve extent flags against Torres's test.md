...
## Correction to the fourth-occurrence note — the detector does have a memory; the passes never used its key (unattended sweep, 2026-08-21)

The 2026-08-19 entry above says "the extent-detector has no memory of prior adjudications". That is wrong, and this pass checked it against the code rather than the tree: `src/ost/extent.ts` declares that an annotation clears an extent issue like every other hygiene rule, and `test/ost/extent.test.ts` pins it — `ost_annotate` with the flag's **exact** issue string removes the flag from the next `computeNextWork`. The four passes that adjudicated these pairs wrote their verdicts in prose, which never matched the key, so the flags returned. This pass annotated all twelve with their exact issue strings (the reasoned DISTINCT verdicts stay beside them), which should end the churn without touching any verdict. The need that made the miss reasonable is mapped at "A flag I already judged false comes back every pass, because the clear is keyed to wording nothing told me to repeat".

This does not change what this test asks: the twelve verdicts are still the sweep's own, and only a human re-applying Torres's test can grade them. It removes the "stop the recurring churn" reason for prioritising it, leaving the real one.
