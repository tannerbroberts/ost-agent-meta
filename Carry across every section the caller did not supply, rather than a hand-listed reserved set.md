...
Named in plain text rather than as a wikilink deliberately. The test's one backlink belongs to its parent assumption, "A rewrite can preserve every unsupplied section without the caller naming one"; a second link from here would fail the `single-backlink` invariant. This pass confirmed that mechanic the hard way — an `ost_edit_node` call that reproduced a node's child links in `prose` produced a duplicate of each, because the tool reattaches the link header itself.
...
## History
- 2026-08-05 merged "Carry across every section the caller did not supply, instead of a hand-listed reserved set" into this node and deleted its file — Same solution, same wording — the original was created earlier in this pass with a fabricated provenance id (`TRANSCRIPT:2026-08-05-unattended-pass`) matching no stored evidence record, reported by `ost_check` as an unresolved-citation violation. `source` is frontmatter with no setter on this surface, so re-creating with truthful free-text provenance and folding the original in is the only available repair. The merge carries the original's edge to its assumption across, and the definition-of-done section is restated below with the assumption's corrected title.
