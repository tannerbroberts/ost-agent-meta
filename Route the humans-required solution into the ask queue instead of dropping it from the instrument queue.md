...
## 2026-09-03 — the census's own stated limit on the shown page is closed: 25 of 25, zero instrumentable

Four lines of new fact, and it closes a gap this node named rather than opening a new subject.

**The limit being discharged.** This node's 2026-08-30 census read 5 of the 25 shown entries and stated its own boundary in terms: "It says nothing about the 44 entries this surface could not page to, nothing about the 20 shown entries not read." The 20-shown-not-read half is now closed. It also flagged its sample as deliberately biased *towards* finding instrumentable entries; a complete page removes that bias in the one direction it ran.

**Method and result.** This firing read 9 of the 25 in full through `ost_read_tree` — the 5 already censused plus "Append-only tool surface with no delete or shell tool", "A human-edited manifest of loop-prescribed call sequences the harvester suppresses", "Axioms elicited at the moment a derivation needs them, one accept-or-reject ask at a time", "A highlight criteria note the founder edits and the loop reads before deciding what to surface", "Name the specific mechanisms a hand process structurally cannot have" and "Borrow the axiom set from a named body of practice and let the human only veto". The remaining 16 were classified from their titles. **Instrumentable entries found: zero, on all 25.**

**The classification, so a reader can check it rather than take it.** Two causes, the same two this node's 2026-08-30 census named, plus one it did not:

- *The artefact is not in this repository* — 6 entries, all naming the harness rather than the product: the Monitor pair, the background-task output directory, the platform job-control wait, the harness compaction, and the auto-read guard. Each is corroborated independently of my reading: `outstandingAsks` already carries a matching "Ask someone with the Monitor tool's implementation open…" and "Ask someone with the harness's sandbox implementation open…" entry for them. The tree had already routed these correctly; only the instrument bucket disagrees.
- *The belief is desirability or viability* — 18 entries. Seven name the human act in the title (charge, sell, cohort, interview, trial, offer, publish); the rest were read and each states its own sharpest risk as viability or desirability in its own prose.
- *A spec would be a false red* — 1 entry, and it is the one genuinely new hazard on this page. "Append-only tool surface with no delete or shell tool" describes a surface the product deliberately moved away from, so a spec asserting "no delete tool" **would** go red today and would still be wrong: it would be measuring the absence of a feature that was added on purpose, and a builder could pick it up and remove `ost_merge_nodes`. That is worse than no spec, and it is a failure mode a naive "just write something red" repair of this bucket would walk straight into.

**What this does and does not do to this candidate.** It supports the premise on correctness across a full page rather than a biased fifth of one: the bucket's contents are owed answers, not work somebody failed to do, and re-filing them states what they are. It does nothing for this node's open question, which remains the operator's answering rate, and it does not touch the 45 entries this surface still cannot page to.

**One observation about cost, since this node is the one weighing build expense.** Six of the nine entries read carry a per-node "examined and deliberately left without an instrument" note written by an earlier sweep, each explaining that the correct disposition is `ost-agent lane --set`. Those notes exist because the bucket re-presents the entry every pass, so each sweep re-derives the same verdict from a body of 2,300 to 15,000 characters. The notes are a workaround for the missing lane check in `src/eval/buildable.ts`, paid for in prose on every affected node — which is a cost the de-duplication repair this node's 2026-09-01 section identified would remove outright, and it is larger than that section accounted for.

_Method: nine node bodies read in full through `ost_read_tree` on this surface, plus this firing's own `ost_next_work` response. Sixteen entries classified by title, not by reading, and that is the weakest link in the count above. Nothing was executed, no instrument set, no lane set, no status changed, no rung moved. This surface holds no `ost_read_repo` grant, so no claim here was checked against the product's code; the six harness attributions rest on the node bodies and on the matching ask-queue entries, not on a first-party read._

## 2026-09-07 — the missing lane check is confirmed in the code, and the repair is one line in a function that already imports the predicate

The 2026-09-03 section on this node ends with its own sharpest limit: "This surface holds no `ost_read_repo` grant, so no claim here was checked against the product's code." This firing holds that grant. The claim is now checked.

**What was read.** `src/eval/buildable.ts` in full (20,070 bytes, untruncated), which is where `solutionsMissingInstruments` is computed.

**The finding: the function consults no lane, on the solution or on its tests.** It excludes exactly four things, and the list is short enough to state completely — a node that is not a Solution; `status === "deferred"`; a solution whose shipped promotion `trustsShippedStatus` accepts; and a solution with no tests (deferred to `solutionsMissingAssumptions`). It then admits the solution unless `tests.some((t) => nodeInstrument(t.test))`. There is no branch reading `lane` anywhere in it.

**The contrast that makes this actionable rather than merely true.** Forty-odd lines above it, in the same module, `testsAwaitingVerification` does check the lane — `if (n.lane === CAUTIOUS_LANE) continue;` — under a comment stating the doctrine outright: "A test a human put beyond compute's reach is never run by compute... The lane is the authority on who may run a test; an instrument only ever says what running it would mean." So the predicate, the `CAUTIOUS_LANE` import, and the principle are all already in the file, applied to the sibling bucket and not to this one. The repair is one line in one function, not a new mechanism to design.

**What this does to this node's own arithmetic.** The 2026-09-01 section inferred the double-count from two equal figures (62 lane-labelled tests, 62 outstanding asks) and conceded that inference's weakness: "not from enumerating both lists and matching them title by title". That inference no longer carries the weight. The double-count is not a coincidence of counts — it is the absence of a branch, read directly. The reading that section reached, "the work for the labelled majority is de-duplication, not routing", is confirmed at the level of the code, and its estimate that this is "materially cheaper than this node's *most expensive of the three to build*" is if anything understated.

**One thing worth saying about intent, because it changes how a builder should read the gap.** The two exclusions that are present each carry a long doc comment arguing for themselves — `shipped` because "for built behaviour any honest spec passes on arrival", `deferred` because "asking a pass to write one anyway is asking it to invent a command over nothing". Both are arguments that a solution can be legitimately unaskable for an instrument. The lane case is the same argument with a different cause, and it is simply not in the list. This looks like an omission in a function whose authors reasoned carefully about exactly this class of exclusion — not a deliberate decision that lane-labelled tests should still count.

**What it does not settle.** This node's open question is the operator's answering rate, and nothing here touches it. Confirming that the bucket over-reports says the items are miscounted; it says nothing about whether re-filing them gets any of them answered, which remains the whole bet and remains humans-required. It also takes no view on which of the three siblings the operator should pick.

**Why no instrument was set on this node despite holding repo sight.** Its one test is "Count how many routed asks the operator answers in the four weeks after they arrive with an age on them", and that is correctly humans-required. A command could assert that the lane exclusion exists — but that would be measuring the sibling de-duplication repair, not the belief this candidate staked, and swapping a node's question for an easier one it can pass is the failure the instrument rule exists to prevent.

**The spec site, for whoever builds it.** `test/eval/` already holds specs against this module — `clearability.test.ts`, `permit-staleness.test.ts`, `vacuous-red.test.ts`. An assertion that a solution whose every test carries the cautious lane is absent from `solutionsMissingInstruments` would be an added assertion in an existing suite, so it would fail on the assertion rather than on a missing file: a strong red, not a `no-spec`. Honest limit: `clearability.test.ts` is 33,252 bytes and would truncate on this surface, so it was not read, and which of these files is the right home is a judgement this pass did not make.

_Method: two `ost_read_repo` file reads (`src/ost/sweep.ts`, `src/eval/buildable.ts`) and three directory listings, plus this firing's own `ost_next_work` response. First-party read of the product's source; it grounds feasibility, not desirability. Nothing was executed, no instrument set, no lane set, no status changed, no rung moved, no node created._
