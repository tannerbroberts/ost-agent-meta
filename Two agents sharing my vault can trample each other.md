...
- 2026-08-07 Near-duplicate of "A second process is editing the same files, and a failed string match is the only notification" — proposed merge, not executed. Both state that concurrent writers to one vault corrupt each other's work. That node already carries three solutions ("A drift sentinel that samples HEAD and file mtimes between steps and interrupts on movement", "Classify the failed match by comparing the file against the run journal's recorded read", "Declare the files a run intends to touch, and refuse to start when another writer already holds them"); this node carries none. Recommended: merge this INTO "A second process is editing the same files, and a failed string match is the only notification", which is the same need stated with the observed symptom attached and is therefore the more useful survivor. Not executed here for want of body sight; see "The repair I am asked to make requires rewriting prose no tool will show me".
- 2026-08-09 2026-08-09 Do not execute the merge proposed on 2026-08-07 — reviewed with body sight and judged wrong. That annotation recommends folding this node INTO "A second process is editing the same files, and a failed string match is the only notification" on the grounds that both "state that concurrent writers to one vault corrupt each other's work", and it says plainly it was written without being able to read the bodies. This pass read both. They are not the same claim, and the merge would flatten a real hierarchy rather than remove a duplicate. This node is a parent over three genuinely distinct concurrency needs, and its own body draws the distinction the proposal misses: "the collision was in the product repo, and the thing that collided was not a write, it was the decision about what to work on". Its children divide that space — "Two runs write the same vault at once and nothing arbitrates between them" is the write race, "A second pass builds what the first already built, because nothing says the work was taken" is the decision collision (which the body explicitly says no vault lease would have prevented), and "A merge conflict got committed into a source file" is the corrupted-state aftermath. The proposed survivor is narrower than all three: it is about detection — learning the ground moved from something that watches, rather than from a failed string match — and it is not about coordination at all. Merging a three-child parent into a node that answers one facet of one child would lose two facets and orphan the structure. The reason this node keeps appearing as under-served is not duplication; it is the counter artifact measured this pass on "An opportunity counts as served when its subtree has solutions, not only its direct children" — 21 of 21 reported under-served opportunities are grouping nodes whose subtrees are already served, and this is one of them. Fixing the counter silences this correctly; merging it silences it by destroying the grouping. What would change this verdict: if a human decides the decision-collision need does not belong under a vault-contention heading, the right repair is to re-parent that child, not to fold this node away.
