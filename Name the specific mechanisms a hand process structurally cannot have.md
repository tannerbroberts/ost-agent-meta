...
## Issues
- 2026-09-01 2026-08-31 unattended sweep, repo sight held: examined for a missing instrument and deliberately left without one. Recorded because this node carries two long audits of its own factual claims but no note about the instrument question, so every firing that meets it in `solutionsMissingInstruments` re-reads 7,900 characters to reach the same answer. The belief beneath it is "Practitioners recognise their own discipline as something that has failed them" — a desirability judgement about whether a listener accepts that their own rigour is the weak part, which the node's own prose already names as the thing to test rather than whether the mechanisms exist. No exit code reaches that. Note the asymmetry this node illustrates well: its four named mechanisms ARE spec-backed and two prior passes verified them first-party, so the feasibility half is answered and un-instrumentable at the same time — a spec here would re-prove what is already green rather than fail today. What a human should do: set the lane with `ost-agent lane --set`, since `ost_flag_humans_required` is withheld on the unattended surface. Not a skipped step. Unchanged by this pass: the standing correction that this node's third pitch bullet ("a status value that does not exist") is false as worded, left for the pitch's owner because rewording a pitch is a positioning call.
