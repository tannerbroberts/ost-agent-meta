...
- 2026-08-05 2026-08-05 `ost_flag_humans_required` is refused at the permission layer on the unattended surface, and this is now the third pass to hit it. Confirmed again this pass: the call was made on "Show readers a degraded run report and see whether they notice" — a test whose own body says "the measurement *is* what a human notices" and "constructing [a mechanical proxy] would answer a different question", which is as clear a case as the tool's `why` argument will ever get — and it returned a permission refusal, not a tool-level refusal. Prior sightings are recorded on that test node (2026-08-05) and against the 2026-08-02 third pass, so it is a standing property of the surface rather than an intermittent one. Why it matters more than a missing convenience. The sweep's instructions name exactly two acceptable dispositions for a solution whose tests are prose-only: write an instrument, or flag the test humans-required. The second is unavailable here, so every solution whose only test is irreducibly a human study has no disposition at all — it cannot be instrumented honestly and it cannot be labelled. Those solutions stay on `solutionsMissingInstruments` permanently and are re-read by every future pass, which is the same self-sustaining backlog shape already documented for stranded evidence on "A Context node type for evidence that is true, useful, and not a customer need". It also biases what unattended passes produce: the only way for a pass to move such a solution off the list is to attach a *new* instrumented test beside the human one, which grows the tree, where the correct act would have shrunk the outstanding list without adding a node. Two passes have now taken the growing option (this one four times) because it was the only one available. Note the asymmetry is the safe way round by design — this tool only ever removes work from compute's reach, which is why an agent is trusted to hold it — so the refusal is very likely a grant that was never added rather than a decision to withhold it. ACTION FOR A HUMAN, one of two: add `mcp__ost-agent__ost_flag_humans_required` to the unattended pass's granted tools, or accept that lane-setting is CLI-only and run `ost-agent lane "<test>" --set humans-required` over the backlog, starting with "Show readers a degraded run report and see whether they notice". Until one of those happens, the count of solutions missing instruments overstates the work a pass can actually do.
- 2026-08-06 Measurement defect, observed 2026-08-06: `solutionsMissingInstruments` appears to count solutions that are correctly humans-required, which means the number cannot reach zero and overstates how much of the tree is failing to become compute-actionable.

The arithmetic from this pass. The bucket read 64 before any writes. The pass then set an instrument on the only test beneath "Refusals the tool can prevent become refusals the tool never issues", and that solution left the list — confirmed by watching it disappear from the alphabetical window. The pass also created three new solutions: two carrying tests with instruments, one carrying a test created with `humansRequired` because an operator's two-week reading is the measurement. After all of it the bucket still read 64. That is 64 − 1 + 1, and the +1 has to be the humans-required solution, because nothing else entered the tree.

Not certain, and here is the gap: the new solution's title sorts into the 39 entries the response does not show, so it was never seen on the list directly. The count is consistent with the inference and no competing explanation was found, but the inference is what it is. What would settle it: raise the display cap, or read the bucket's own predicate.

Why it belongs on this node rather than filed as tidiness. This opportunity is about knowing what compute can act on without a human. The bucket is the closest thing the tree has to that measurement, and if it conflates "nobody has written the command yet" with "no command can exist", then the one number an operator would use to judge compute-actionability is answering a different question from the one being asked. A pass reading 64 concludes there are 64 commands left to write. If a substantial share of them are willingness-to-pay studies and moderated usability sessions, the true figure is much smaller and much more tractable, and the tree looks worse than it is.

The fix is a distinction, not a threshold: count solutions with no runnable definition of done *and* no humans-required test separately from solutions whose definition of done is legitimately a person. Both are useful. Added together they are neither.
