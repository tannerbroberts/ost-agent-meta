...
- 2026-08-23 2026-08-23 — correction to a figure in the standing finding above, from a full triage rather than an estimate. That section says "roughly eleven" of the 25 visible `solutionsMissingInstruments` entries rest on irreducibly human tests. This pass held repo sight and read all 25 down to their assumption and test nodes. The honest count is far higher: **24 of 25 cannot take an honest instrument on this surface.** Eleven are market/desirability tests that only people can answer (cold offers, willingness-to-pay probes, story-based interviews, practitioner recognition, "two unattended weeks", whether the founder maintains a criteria note or judges a digest complete). Six are questions about the *harness*, not this repository — the Write/Edit read-precondition, Monitor's command grammar, a background task's output directory, a backgrounding lifecycle hook — and no spec in `test/` can reach them; two nodes already carry that routing correction, written by earlier passes. Six more are behavioural questions about whether a model or an operator keeps something current, and one ("Append-only tool surface with no delete or shell tool") is documented on its own node as having no honest instrument, because a spec asserting "no delete tool" would go red for a feature that was added on purpose. The single partial exception is "Ship the helper with its own runtime rather than borrowing the machine's", whose test asks for a length *and readability* comparison — the first half is mechanical, the second is a person's judgement, so instrumenting it would answer half the question and report the whole. **Why this matters for how the counter is read:** `solutionsMissingInstruments` is being read as a backlog of work an unattended pass declined to do. On this evidence it is very nearly the human-lane share of the tree wearing the wrong label, and it carries that label precisely because `ost_flag_humans_required` — the one call that would dispose of it correctly — is withheld here. The number does not go down by trying harder; it goes down by granting the lane tool or by counting these separately. Bound on this claim: it covers the 25 the response displays, not the 39 it truncates.
