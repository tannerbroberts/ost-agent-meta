...
## Issues
- 2026-08-03 Lane label could not be written — `ost_flag_humans_required` was declined at the permission layer on the 2026-08-03 third unattended sweep (fourth consecutive pass this tool has been refused). This test needs a person by construction: the measurement is what the operator ratifies and how long they spend, and no reading of committed state substitutes for it. Until a human sets the lane with `ost-agent lane`, it will report under `needsHumans` for the right reason by accident rather than by label, and an attended session has no way to tell it apart from a test nobody has classified. A human should set it, and should also rule on the population question in the body — one operator means this measures the founder's tolerance, not an operator's.
