...
## Issues
- 2026-08-22 2026-08-22 (unattended sweep, repo sight held) — THIS SCAN WOULD MEASURE NOTHING LOAD-BEARING, per source. `src/knowledge/dispositions.ts` states that "body citations are invisible to the sweep" — an evidence item leaves `unmappedEvidence` only when a node's frontmatter `source:` equals its id. So body-text hits for an unmapped id discharge nothing by construction, and counting them cannot explain why an item is or is not on the list. If the intent was to find out how records leave the list, that is answered: frontmatter `source:` only, and no tool can add one to an existing node. Not recorded as a result — this pass read source without executing anything, and recording a result is a human's `ost-agent result`. Re-scope or retire this test rather than running it.
