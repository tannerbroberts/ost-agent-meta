...
## What separates this bucket from "I can't leave the process running unattended without worrying"

**This bucket is about the run's conduct and output being believable. The sibling is about the run continuing to work at all.** The mirror of the section written on that node the same pass; recorded on both sides so a reader arriving at either can route a child without opening the other.

The boundary, in one line each:

- **Here:** can I believe what it did while nobody was watching, and could it have done something I would not sanction? Grading its own homework, unproven work dressed as proven, leaking a connected system of record, a destructive irreversible action, no account of the time I was away. The failure mode is *the run worked perfectly and I still cannot trust the result.*
- **There:** did the machinery keep running, and can it be restarted without damage? Crashes, locks, half-writes, stale leftovers, denied tool grants, merge conflicts, a loop re-selecting deferred work. The failure mode is *the run stopped, stalled, or corrupted itself.*

**Torres's test, applied 2026-08-22 rather than assumed.** These pass as distinct siblings: an independent judge that grounds the agent's claims answers this bucket and does nothing for continuity; "Resumable append-only process journal" answers continuity and does nothing for honesty. **The merge reading of the 2026-07-25 flag on the sibling therefore fails** — the open question is naming, which no agent surface can act on, not merging.

**A caution for whoever renames these.** The honest/continuity split is clean at the child level but the two do interact in one place, and it is worth not losing: a run that dies silently is a continuity failure that *presents* as a trust failure, because the operator comes back to a tree that is simply wrong with nothing saying why. "A failed pass reports success, so my automation can't tell" sits on the sibling for that reason and could defensibly sit here. It is the one child where the boundary is a judgement rather than a reading.
...
- 2026-08-22 The 2026-07-25 growth hold above was respected again by this unattended sweep: no node was created under this branch. The prose edit adding the differentia section is not growth — it adds no child and makes no new claim about the need, it records a boundary the sibling's own four-week-old flag asked for.
...
- 2026-08-22 body edited — The near-synonym flag on the sibling node ("I can't leave the process running unattended without worrying") has been open since 2026-07-25 with neither node's prose stating what separates them, so the boundary was rediscoverable only by reading both sets of children. Recording the differentia on this side too, so a reader arriving at either node can route a child without opening the other, and noting that the merge half of that flag was tested and fails. No prior claim removed; both existing Issues entries preserved verbatim.
