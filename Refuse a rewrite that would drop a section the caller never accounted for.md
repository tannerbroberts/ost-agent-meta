...
**Compared with its siblings.** "Carry across every section the caller did not supply, rather than a hand-listed reserved set" makes the mistake unrepresentable and asks nothing of the caller, but also removes the ability to delete. This keeps deletion available and makes it deliberate, at the price of assuming the caller will read a refusal and act on it — an assumption the vault's own census of thirteen sessions rediscovering one refusal should make nobody comfortable. "Report what a write changed, so a silent loss stops being silent" gives up on prevention entirely in exchange for covering losses in tools nobody has audited.
...
Named in plain text rather than as a wikilink: the test's one backlink belongs to its parent assumption, "A guard can catch the unacknowledged drop without refusing honest rewriting".
...
## History
- 2026-08-05 merged "Refuse a rewrite that would drop a section the caller never acknowledged" into this node and deleted its file — Same solution, same wording — the original was created earlier in this pass with a fabricated provenance id (`TRANSCRIPT:2026-08-05-unattended-pass`) matching no stored evidence record, reported by `ost_check` as an unresolved-citation violation. `source` is frontmatter with no setter on this surface, so re-creating with truthful free-text provenance and folding the original in is the only available repair. The merge carries the original's edge to its assumption across, and the definition-of-done section is restated below with corrected titles.
