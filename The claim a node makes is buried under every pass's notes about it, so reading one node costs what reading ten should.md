...
## 2026-09-16 — consolidation is possible after all, and it does not hold; the re-accretion rate is now measured

Short, per this node's own convention, because a long section here refutes its own subject.

**One claim on this node is false, and it is load-bearing.** The 2026-09-08 section concludes: "a node past the read cap can no longer be consolidated by the tool built to consolidate it. It is append-only in practice and can only grow," and the paragraph after it calls that "decisive rather than merely unfortunate" for the fork this node hands a human. It was refuted the same day, on another node, by a pass that did the thing it says cannot be done. "Work I already decided needs a person comes back every pass as work I failed to do" was consolidated on 2026-09-08 from 79,217 prose characters, dropping fourteen dated sections, and its own Issues note records the route in general form: the truncation is a property of `ost_read_tree`, not of the vault — the vault's nodes are ordinary Markdown and the harness's `Read` and `Grep` reach them whole, so the hidden headings can be enumerated after all. This node has carried the false premise for eight days.

**What replaces it is worse for the same fork, and it is a measurement rather than a reading.** Consolidation is composable; it simply does not hold. Counted first-party this pass by grepping `^## ` over all three nodes whose `## History` records a consolidation:

| Node | Consolidated | Sections dropped | Dated sections since | Days |
|---|---|---|---|---|
| "Work I already decided needs a person comes back every pass as work I failed to do" | 2026-09-08 | 14 | **13** | 8 |
| "A human-edited manifest of loop-prescribed call sequences the harvester suppresses" | 2026-09-11 | 5 | **4** | 5 |
| "Append-only tool surface with no delete or shell tool" | 2026-08-21 | 7 | **0** | 26 |

The first regrew to 101,229 prose characters — past the 79,217 its own repair was applied to — in eight days, two of the thirteen sections landing today before this firing. The second stands at 38,585 against the 42,319 it was repaired from. That is roughly 1.6 and 0.8 sections per day against removal rates of fourteen and five in a single write.

**The discriminator is what the node's content is, not how large it had grown.** The two that regrew each carry a *standing census* — an open count that every pass is explicitly invited to add a data point to — and all thirteen and all four of the new sections do exactly that. The one that held carries a closed (a)/(b) question with nothing a pass can add to it. So consolidation is not a repair with a half-life; it is a repair that works on a node with no open counter and fails on a node with one. That is a third option, and it is neither branch of the fork: retire the counter, not the sections.

**What this does to the fork.** The conclusion the 2026-09-08 section drew survives its false premise and is now better argued. The read-boundary repair still loses — not because consolidation is impossible past the cap, but because it is available, was performed twice in eight days, and was undone faster than it was applied. A write-boundary rule is the only one of the two that acts on the rate rather than on the stock.

**Limits.** Three nodes, all of this vault, chosen because they are the only ones whose History records a consolidation — not a sample, and n=1 in the held column. Post-consolidation sizes are not recoverable from disk, so "regrew past its pre-repair size" is bounded against the pre-repair figure rather than measured from the trough, which is why the second node's growth is stated as a section count instead. Section counts use a `## ` heading carrying a date, the same proxy this node's own census names as a floor, so a dated note written without a heading is missed. The content discriminator is a judgement over three nodes, not a fitted result. This is the product's own agent measured against itself — it grounds usability, not desirability, and is not evidence that anyone outside this project wants shorter nodes. Nothing was executed, no rung moved, no instrument set, no status changed, no node created. `ost_check` is withheld on this surface, so this write is unverified by the invariant checker by design.

_Method: read-only `Grep` for `^## ` over the three node files, their own `## History` entries for the pre-repair figures, and this pass's own `ost_read_tree` responses for prose sizes. Observed structure of this vault, read first-party._

## 2026-09-16 — USAGE:2026-09-16 corroborates, briefly

`USAGE:2026-09-16` (machine-recorded): 319 calls across 22 sessions. Only 9 were writes (8 `ost_append_to_node`, 1 `ost_annotate`), while `ost_read_tree` took 158. That's under one write per two sessions. This unattended firing saw the cause directly. Four of the 75 `solutionsMissingInstruments` solutions it opened had long bodies, one at 27,902 characters, most of it earlier passes explaining why they set no instrument. A lane nobody set would have said the same thing in one field. Read, not run; nothing else changed. `ost_check` is withheld on this surface, so this write isn't verified.
